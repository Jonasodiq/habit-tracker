...
export default function CreateHabitScreen() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      router.back();
        ...
        const msg = err?.response?.data?.error || 'Kunde inte skapa vanan';
        Alert.alert('Fel', msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      <Text style={styles.title}>Ny vana</Text>
      ...
      {/* Spara-knapp */}
      <TouchableOpacity style={[styles.button, { backgroundColor: color }]} onPress={handleCreate} disabled={loading}>
        {loading ? (
          <ActivityIndicator color="#fff" />
        ) : (
          <Text style={styles.buttonText}>Skapa vana</Text>
        )}
      </TouchableOpacity>
